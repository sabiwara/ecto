defmodule Ecto.ChangesetTest do
  ...
  defp changeset(schema \\ %Post{}, params) do
    cast(
      schema,
      params,
      ~w(id token title author_email body upvotes decimal color topics virtual)a
    )
  end
    ...
    if Version.match?(System.version(), ">= 1.13.0-dev") do
      test "generates correct where clause for single primary key without query option" do
        ...
        assert_receive [
          MockRepo,
          function: :exists?,
          query: %Ecto.Query{wheres: wheres},
          opts: []
        ]
  ...
  test "foreign_key_constraint/3" do
    ...
    changeset =
      change(%Comment{})
      |> foreign_key_constraint(:post, name: :whatever, match: :exact, message: "is not available")
