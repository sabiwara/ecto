defmodule Ecto.Query.Planner do
  ...
  defp normalize_subquery_select(query, adapter, source?) do
    ...
    {{:map, types}, _fields, _from} =
      collect_fields(expr, [], :never, query, select.take, true, %{})
      ...
      Enum.reduce(with_expr.queries, {[], counter}, fn
        {name, %Ecto.Query{} = inner_query}, {queries, counter} ->
          ...
          {{:map, types}, fields, _from} =
            collect_fields(expr, [], :never, inner_query, take, true, %{})
  ...
  defp collect_fields(
    ...
    {right, right_fields, _from} =
      collect_fields(right, [], from, query, take, keep_literals?, %{})
